...
UNIVERSAL_ROUTER_JSON_FILE=abstract-$NETWORK.json
DO_ZK_SYNC=true
DO_BROADCAST=false
BROADCAST_FLAG=""
ZK_SYNC_FLAG=""
...
if [ "$DO_ZK_SYNC" = true ]; then
    ZK_SYNC_FLAG="--zksync"
fi

# Validate forge version includes foundry-zksync
echo 'Validating forge version...'
if ! forge --version | grep -q "foundry-zksync"; then
    echo "ERROR: forge version must include 'foundry-zksync'"
    echo "Current forge version:"
    forge --version
    exit 1
fi
echo 'Forge version validation passed'
...
echo 'DO_ZK_SYNC: '$DO_ZK_SYNC
echo 'ZK_SYNC_FLAG: '$ZK_SYNC_FLAG
...
export OWNER=$OWNER
export FINAL_OWNER=$FINAL_OWNER
export SALT=$SALT
export WETH9=$WETH9
...
if [ "$CHECK_UNISWAP_V3_POOL_INIT_CODE_HASH" = true ]; then
    forge script script/00_PunchSwapV3PoolInitBytecodeHash.s.sol $ZK_SYNC_FLAG --rpc-url $RPC_URL --slow --legacy | tee -a ../local_deploy.txt
fi
...
if [ "$DEPLOY_CORE" = true ]; then
    forge script script/01_PunchSwapV3Factory.s.sol $ZK_SYNC_FLAG --rpc-url $RPC_URL --slow --legacy $BROADCAST_FLAG | tee -a ../local_deploy.txt
...
if [ "$DEPLOY_PERIPHERY_1" = true ]; then
    forge script script/03_PunchSwapInterfaceMulticall.s.sol $ZK_SYNC_FLAG --rpc-url $RPC_URL --slow --legacy $BROADCAST_FLAG | tee -a ../local_deploy.txt
    forge script script/04_TickLens.s.sol $ZK_SYNC_FLAG --rpc-url $RPC_URL --slow --legacy $BROADCAST_FLAG | tee -a ../local_deploy.txt
    forge script script/05_Quoter.s.sol $ZK_SYNC_FLAG --rpc-url $RPC_URL --slow --legacy $BROADCAST_FLAG | tee -a ../local_deploy.txt
        # V3 Factory
        # WETH9
    forge script script/05a_PunchSwapV3StaticQuoter.s.sol $ZK_SYNC_FLAG --rpc-url $RPC_URL --slow --legacy $BROADCAST_FLAG | tee -a ../local_deploy.txt
        # V3 Factory
    forge script script/06_SwapRouter.s.sol $ZK_SYNC_FLAG --rpc-url $RPC_URL --slow --legacy $BROADCAST_FLAG | tee -a ../local_deploy.txt
        # V3 Factory
        # WETH9
    forge script script/07_QuoterV2.s.sol $ZK_SYNC_FLAG --rpc-url $RPC_URL --slow --legacy $BROADCAST_FLAG | tee -a ../local_deploy.txt
        # V3 Factory
        # WETH9
    if [ "$NETWORK" = "mainnet" ]; then
        forge script script/08_NonfungibleTokenPositionDescriptor.s.sol $ZK_SYNC_FLAG --rpc-url $RPC_URL --slow --legacy --sender $OWNER $BROADCAST_FLAG | tee -a ../local_deploy.txt
            # ETH_NATIVE_CURRENCY_LABEL_BYTES
    else
        forge script script/08a_TestnetNonfungibleTokenPositionDescriptor.s.sol $ZK_SYNC_FLAG --rpc-url $RPC_URL --slow --legacy $BROADCAST_FLAG --sender $OWNER | tee -a ../local_deploy.txt
            # ETH_NATIVE_CURRENCY_LABEL_BYTES  
    fi
else
...
if [ "$DEPLOY_PERIPHERY_2" = true ]; then
    forge script script/09_NonfungiblePositionManager.s.sol $ZK_SYNC_FLAG --rpc-url $RPC_URL --slow --legacy $BROADCAST_FLAG | tee -a ../local_deploy.txt
        # WETH9
        # V3FACTORY
        # TOKEN_DESCRIPTOR
else
...
if [ "$DEPLOY_PERIPHERY_3" = true ]; then
    forge script script/10_V3Migrator.s.sol $ZK_SYNC_FLAG --rpc-url $RPC_URL --slow --legacy $BROADCAST_FLAG | tee -a ../local_deploy.txt
        # WETH9
        # V3FACTORY
        # POSITION_MANAGER
else
...
if [ "$DEPLOY_SWAP_ROUTER" = true ]; then
    forge script script/11_SwapRouter02.s.sol $ZK_SYNC_FLAG --rpc-url $RPC_URL --slow --legacy $BROADCAST_FLAG | tee -a ../local_deploy.txt
else
...
if [ "$DEPLOY_UNIVERSAL_ROUTER" = true ]; then
    forge script script/12_UniversalRouter.s.sol $ZK_SYNC_FLAG --sig "runAndDeployPermit2(string)" script/deployParameters/$UNIVERSAL_ROUTER_JSON_FILE --rpc-url $RPC_URL --slow --legacy $BROADCAST_FLAG | tee -a ../local_deploy.txt
else
...
if [ "$DEPLOY_STAKER" = true ]; then
    forge script script/13_PunchSwapV3Staker.s.sol $ZK_SYNC_FLAG --rpc-url $RPC_URL --slow --legacy $BROADCAST_FLAG | tee -a ../local_deploy.txt
else
...
if [ "$DEPLOY_FEE_COLLECTOR" = true ]; then
    forge script script/14_FeeCollector.s.sol $ZK_SYNC_FLAG --rpc-url $RPC_URL --slow --legacy $BROADCAST_FLAG | tee -a ../local_deploy.txt
else
