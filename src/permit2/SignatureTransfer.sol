...
contract SignatureTransfer is ISignatureTransfer, EIP712 {
    ...
    function permitWitnessTransferFrom(
    ...
    ) external {
        {
            _permitTransferFrom(
                // permit, transferDetails, owner, permit.hashWithWitness(witness, witnessTypeString), signature
                permit, transferDetails, owner, _permitHashWithWitness(permit, witness, witnessTypeString), signature
            );
        }
    }

    function _permitHashWithWitness(PermitBatchTransferFrom memory permit, bytes32 witness, string calldata witnessTypeString) internal view returns (bytes32) {
