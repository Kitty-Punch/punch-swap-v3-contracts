...
import '../../core/interfaces/callback/IPunchSwapV3FlashCallback.sol';
...
/// @notice An example contract using the PunchSwap V3 flash function
contract PairFlash is IPunchSwapV3FlashCallback, PeripheryPayments {
    ...
    /// @notice Calls the pools flash function with data needed in `punchSwapV3FlashCallback`
    function initFlash(FlashParams memory params) external {
        PoolAddress.PoolKey memory poolKey =
            PoolAddress.PoolKey({token0: params.token0, token1: params.token1, fee: params.fee1});
        IPunchSwapV3Pool pool = IPunchSwapV3Pool(PoolAddress.computeAddress(factory, poolKey));
