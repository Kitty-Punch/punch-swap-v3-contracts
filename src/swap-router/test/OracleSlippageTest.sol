...
contract OracleSlippageTest is OracleSlippage {
    mapping(address => mapping(address => mapping(uint24 => IPunchSwapV3Pool))) private pools;
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view override returns (IPunchSwapV3Pool pool) {
        pool = pools[tokenA][tokenB][fee];
    }

    function testGetBlockStartingAndCurrentTick(IPunchSwapV3Pool pool)
        external
        view
