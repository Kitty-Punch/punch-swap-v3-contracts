...
if [ "$NETWORK" = "abstract-testnet" ]; then
    VERIFIER_URL="https://api-explorer-verify.testnet.abs.xyz/contract_verification"
else
    VERIFIER_URL="https://api-explorer-verify.mainnet.abs.xyz/contract_verification"
fi

VERIFY_FLAG="--verify --verifier zksync --verifier-url $VERIFIER_URL"

if [ "$DO_BROADCAST" = true ]; then
    BROADCAST_FLAG="--broadcast"
fi

if [ "$DO_ZKSYNC" = true ]; then
    ZKSYNC_FLAG="--zksync"
fi

echo 'NETWORK: '$NETWORK
echo 'VERIFIER_URL: '$VERIFIER_URL
echo 'VERIFY_FLAG: '$VERIFY_FLAG
...
if test "clean" = "$1"; then
    forge clean && forge cache clean all && forge build --force $ZKSYNC_FLAG
else 
    forge build $ZKSYNC_FLAG
fi

if [ "$CHECK_UNISWAP_V3_POOL_INIT_CODE_HASH" = true ]; then
    forge script $ZKSYNC_FLAG script/00_PunchSwapV3PoolInitBytecodeHash.s.sol --rpc-url $RPC_URL --chain $NETWORK $VERIFY_FLAG $BROADCAST_FLAG | tee -a ../local_deploy.txt
fi
...
if [ "$DEPLOY_CORE" = true ]; then
    echo forge script script/01_PunchSwapV3Factory.s.sol --rpc-url $RPC_URL --chain $NETWORK $VERIFY_FLAG $BROADCAST_FLAG
    forge script script/01_PunchSwapV3Factory.s.sol --rpc-url $RPC_URL --chain $NETWORK $VERIFY_FLAG $BROADCAST_FLAG | tee -a ../local_deploy.txt
...
# forge script script/02_KonaPointsFarmDeploy.s.sol:KonaPointsFarmDeploy --rpc-url https://api.testnet.abs.xyz --chain abstract-testnet --zksync --verify --verifier zksync --verifier-url https://api-explorer-verify.testnet.abs.xyz/contract_verification --broadcast 

if [ "$DEPLOY_PERIPHERY_1" = true ]; then
    forge script script/03_PunchSwapInterfaceMulticall.s.sol --rpc-url $RPC_URL --chain $NETWORK $VERIFY_FLAG $BROADCAST_FLAG | tee -a ../local_deploy.txt
    forge script script/04_TickLens.s.sol --rpc-url $RPC_URL --chain $NETWORK $VERIFY_FLAG $BROADCAST_FLAG | tee -a ../local_deploy.txt
    forge script script/05_Quoter.s.sol --rpc-url $RPC_URL --chain $NETWORK $VERIFY_FLAG $BROADCAST_FLAG | tee -a ../local_deploy.txt
        # V3 Factory
        # WETH9
    forge script script/05a_PunchSwapV3StaticQuoter.s.sol --rpc-url $RPC_URL --chain $NETWORK $VERIFY_FLAG $BROADCAST_FLAG | tee -a ../local_deploy.txt
        # V3 Factory
    forge script script/06_SwapRouter.s.sol  --rpc-url $RPC_URL --chain $NETWORK $VERIFY_FLAG $BROADCAST_FLAG | tee -a ../local_deploy.txt
        # V3 Factory
        # WETH9
    forge script script/07_QuoterV2.s.sol  --rpc-url $RPC_URL --chain $NETWORK $VERIFY_FLAG $BROADCAST_FLAG | tee -a ../local_deploy.txt
        # V3 Factory
        # WETH9
    if [ "$NETWORK" = "abstract" ]; then
        forge script script/08_NonfungibleTokenPositionDescriptor.s.sol  --rpc-url $RPC_URL --chain $NETWORK $VERIFY_FLAG $BROADCAST_FLAG | tee -a ../local_deploy.txt
            # ETH_NATIVE_CURRENCY_LABEL_BYTES
    else
        forge script script/08a_TestnetNonfungibleTokenPositionDescriptor.s.sol  --rpc-url $RPC_URL --chain $NETWORK $VERIFY_FLAG $BROADCAST_FLAG --sender $OWNER | tee -a ../local_deploy.txt
            # ETH_NATIVE_CURRENCY_LABEL_BYTES  
    fi
else
...
if [ "$DEPLOY_PERIPHERY_2" = true ]; then
    forge script script/09_NonfungiblePositionManager.s.sol --rpc-url $RPC_URL --chain $NETWORK $VERIFY_FLAG $BROADCAST_FLAG | tee -a ../local_deploy.txt
        # WETH9
        # V3FACTORY
        # TOKEN_DESCRIPTOR
else
...
if [ "$DEPLOY_PERIPHERY_3" = true ]; then
    forge script script/10_V3Migrator.s.sol --rpc-url $RPC_URL --chain $NETWORK $VERIFY_FLAG $BROADCAST_FLAG | tee -a ../local_deploy.txt
        # WETH9
        # V3FACTORY
        # POSITION_MANAGER
else
...
if [ "$DEPLOY_SWAP_ROUTER" = true ]; then
    forge script script/11_SwapRouter02.s.sol --rpc-url $RPC_URL --chain $NETWORK $VERIFY_FLAG $BROADCAST_FLAG | tee -a ../local_deploy.txt
else
...
if [ "$DEPLOY_UNIVERSAL_ROUTER" = true ]; then
    forge script script/12_UniversalRouter.s.sol --sig "runAndDeployPermit2(string)" script/deployParameters/$UNIVERSAL_ROUTER_JSON_FILE --rpc-url $RPC_URL --chain $NETWORK $VERIFY_FLAG $BROADCAST_FLAG | tee -a ../local_deploy.txt
else
...
if [ "$DEPLOY_STAKER" = true ]; then
    forge script script/13_PunchSwapV3Staker.s.sol --rpc-url $RPC_URL --chain $NETWORK $VERIFY_FLAG $BROADCAST_FLAG | tee -a ../local_deploy.txt
else
...
if [ "$DEPLOY_FEE_COLLECTOR" = true ]; then
    forge script script/14_FeeCollector.s.sol --rpc-url $RPC_URL --chain $NETWORK $VERIFY_FLAG $BROADCAST_FLAG | tee -a ../local_deploy.txt
else
